...
class DrawHorizontalWinningLine:
    instance = None

    def __new__(cls, *args, **kwargs):
        if not cls.instance:
            cls.instance = super(DrawHorizontalWinningLine, cls).__new__(cls)
    ...
    def display(self, info):
        for index, row in enumerate(info.w_values):
            line_start_x = (self.instance.width // 2) + (self.instance.square_size // 2)
            line_start_y = PADDING + (self.instance.square_size // 2) + row * self.instance.square_size
            ...
            pygame.draw.line(self.instance.screen, color, (line_start_x, line_start_y),
                             (line_end_x, line_start_y), LINE_WIDTH)
