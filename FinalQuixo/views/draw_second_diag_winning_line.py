...
class DrawSecondDiagWinningLine:
    instance = None

    def __new__(cls, *args, **kwargs):
        if not cls.instance:
            cls.instance = super(DrawSecondDiagWinningLine, cls).__new__(cls)
    ...
    def display(self, info):
        for index, diag in enumerate(info.w_values):
            gap_x = (self.instance.width // 2) + (self.instance.square_size // 2)
            line_start_x = gap_x + self.instance.quixo_format * self.instance.square_size - self.instance.square_size
