...
class DrawVerticalWinningLine:
    instance = None

    def __new__(cls, *args, **kwargs):
        if not cls.instance:
            cls.instance = super(DrawVerticalWinningLine, cls).__new__(cls)
    ...
    def display(self, info):
        for index, col in enumerate(info.w_values):
            line_start_x = (self.instance.width // 2) + (
                        self.instance.square_size // 2) + col * self.instance.square_size
            line_start_y = PADDING + (self.instance.square_size // 2)
            line_end_y = line_start_y + (
                        self.instance.quixo_format * self.instance.square_size) - self.instance.square_size
